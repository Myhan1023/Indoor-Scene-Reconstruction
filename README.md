...
本项目是一个从底层开发的计算机视觉与图形学交叉系统，旨在解决室内环境从 2D 语义映射到 3D 物理空间的自动化重建问题。系统脱离了商业引擎（如 Unity/UE）的封装，完全基于 C++ 与 OpenGL 核心态开发，为具身智能（Embodied AI）中的环境感知与高保真仿真提供了底层技术支撑。
...
### 3. GPU 片段着色器深度优化 (Shader Optimization)
* **偏导数抗畸变方案**：针对大面积低采样网格在高光下产生的插值畸变（Pinch Artifacts），本项目在 Fragment Shader 中通过 **GPU 屏幕空间偏导数 (`dFdx`, `dFdy`)** 实时重构面法线。
* **性能表现**：该方案实现了零 CPU 额外开销下的平滑着色（Flat Shading），在普通移动端显卡上即可稳定保持 60+ FPS 的实时渲染性能。
